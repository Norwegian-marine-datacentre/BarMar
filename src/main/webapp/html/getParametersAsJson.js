function getParametersAsJson(grid, species, speciesSubGroup, time, depth, displaytype){
    ...
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            var dropDownAdd = function( buttonId, optionTxt, index, selectedValue ) {
                if ( index === 0 ) {
                    $( buttonId ).html( '<li><a href="#" data-value="' + optionTxt + '">' + optionTxt + '</a></li>' );
                    console.log("enable btn:"+buttonId);
                    $(buttonId+'Btn').attr('disabled', false);
                } else {
                    $( buttonId ).append( '<li><a href="#" data-value="' + optionTxt + '">' + optionTxt + '</a></li>' );
                }
                if ( optionTxt === selectedValue) {
                    setSelectedValue( buttonId, optionTxt );
                } else if ( selectedValue === undefined ) {
                    setSelectedValue( buttonId, buttonId );
                }
            };
            ...
            if ( speciesSubgroupList == null || speciesSubgroupList.length == 0 ) {
                setSelectedValue( "#speciesSubGroup", "Select Value" );
                $('#speciesSubGroupBtn').attr('disabled', true);
            ...
            if ( depthList == null || depthList.length == 0 ) {
                setSelectedValue( "#depth", "Select Value" );
                $('#depthBtn').attr('disabled', true);
                $('#depth').html( '' );
            }
            ...
            if ( periodList == null || periodList.length == 0 ) {
                setSelectedValue( "#period", "Select Value" );
                $('#periodBtn').attr('disabled', true);
                $('#period').html( '' );
            }
