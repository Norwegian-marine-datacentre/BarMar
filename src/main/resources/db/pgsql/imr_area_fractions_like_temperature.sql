-- Function: imr_area_fractions_like_temperature(character varying, character varying, character varying)

-- DROP FUNCTION imr_area_fractions_like_temperature(character varying, character varying, character varying);

CREATE OR REPLACE FUNCTION imr_area_fractions_like_temperature(
...
$BODY$
  ...
  BEGIN
     ...
     ELSE
	   ...
	   --
	   select st_area2d(transform(gridcellpolygon_from,3035))/1000000.0 into area_gridcellpolygon_from;
       ...
       LOOP
		 ...
		 ELSE
		   select st_area2d(transform(gridcellpolygon_from,3035))/1000000.0 into area_gridcellpolygon_to;
		   IF st_intersects(gridcellpolygon_from,gridcellpolygon_to) THEN
             IF st_within(gridcellpolygon_to,gridcellpolygon_from) THEN
			 ...
			 ELSE
	           ...
	           WHILE (i_polygon <= n_polygon) LOOP
                 select st_geometryn(gridcellpolygon_to,i_polygon) into a_polygon;
		       ...
		       END LOOP;	   
			   output := output || gridcellpolygon_to_name || ',' || trim(to_char(overlap_area/area_gridcellpolygon_to,'0.99999')) || ';';
			 END IF;
		   END IF;
		 END IF;		 
	   END LOOP;	   	   
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION imr_area_fractions_like_temperature(character varying, character varying, character varying)
  OWNER TO grid_owner;
GRANT EXECUTE ON FUNCTION imr_area_fractions_like_temperature(character varying, character varying, character varying) TO grid_owner;
GRANT EXECUTE ON FUNCTION imr_area_fractions_like_temperature(character varying, character varying, character varying) TO public;
