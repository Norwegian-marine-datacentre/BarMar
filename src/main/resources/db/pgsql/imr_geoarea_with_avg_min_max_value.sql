-- Function: imr_geoarea_with_avg_min_max_value(text, text, text, text, text)

-- DROP FUNCTION imr_geoarea_with_avg_min_max_value(text, text, text, text, text);

CREATE OR REPLACE FUNCTION imr_geoarea_with_avg_min_max_value(
    IN agridname text,
    IN parameter_ids text,
    IN depthlayernametext text,
    IN periodnametext text,
    IN aggregationfunc text)
  RETURNS TABLE(id bigint, geoshape geometry, gridname character varying, gridcellname character varying, parametername character varying, parameter_id bigint, depthlayername character varying, periodname character varying, value double precision) AS
$BODY$	
    DECLARE
      sql text;
    ...
    BEGIN
     sql := $$select 
	DISTINCT ON (h.name) v.id, h.geoshape, g.name as gridname, h.name as gridcellname, p.name as parametername, p.id as parameter_id, vc.name as depthlayername, tc.name as periodname, $$;
	...
	sql := sql || $$ cast(min(v.value) OVER (PARTITION BY h.name) as double precision) as value $$;
     ELSIF aggregationFunc = 'max' THEN
	sql := sql || $$ cast(max(v.value)  OVER (PARTITION BY h.name) as double precision) as value $$;
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION imr_geoarea_with_avg_min_max_value(text, text, text, text, text)
  OWNER TO grid_owner;
